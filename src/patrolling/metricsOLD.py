...
class MetricsOLD:
    ...
    def append_statistics_on_target_reached(self, action, learning_tuple=None):
        ...
        if learning_tuple is not None:
            ...
            if len(self.targets_threshold.keys()) == 0:
                self.targets_threshold = {t.identifier: t.maximum_tolerated_idleness for t in
                                          self.simulator.environment.targets}
    ...
    def save_dataframe_light(self):
        ...
        df_dqn.to_csv(self.simulator.directory_simulation() + "dqn_training_data.csv", mode='a',
                      header=has_header)
    ...
    def save_dataframe(self):
        ...
        target_aoi_columns = ["target_{}_aoi".format(t.identifier) for t in
                              self.simulator.environment.targets]

        df_tar = pd.DataFrame()

        df_tar["cur_second"] = pd.Series(self.cur_second)
        df_tar["date"] = pd.Series(
            [pd.to_datetime('now').normalize().strftime("%d-%m-%Y %H:%M:%S")] * N_ROWS)
        df_tar["date"] = pd.to_datetime(df_tar["date"]) + pd.Series(
            [pd.to_timedelta(delta, unit='s') for delta in self.cur_second])
        ...
        df_tar.to_csv(self.simulator.directory_simulation() + "log_simulation.csv", mode='a',
                      header=has_header)
        ...
        if has_header:
            ...
            # MERGE
            df_qvalues_disk = pd.read_csv(self.simulator.directory_simulation() + "qvalues.csv",
                                          index_col=0)
            ...
            merged = merged.rename(
                columns={other_columns_x[i]: other_columns[i] for i in range(len(other_columns_x))})
