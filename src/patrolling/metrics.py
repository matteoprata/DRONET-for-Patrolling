...
class Metrics:
    ...
    def plot(self, time, ROLLING_WINDOW=50):
        json_to_save = {"AOIS": self.aois, "LOSSES": self.losses, "REWS": self.rewards}
        utilities.save_json(json_to_save, config.RL_DATA + "data-{}.json".format(self.simulator.simulation_name()))
        ...
        if config.DRONE_MOBILITY == config.Mobility.DECIDED:
            ...
            self.fig.savefig(config.RL_DATA + "loss-training-{}.png".format(self.simulator.simulation_name()))
            plt.clf()
            ...
            self.fig.savefig(config.RL_DATA + "rew-training-{}.png".format(self.simulator.simulation_name()))
            plt.clf()

            plt.plot(range(len(self.epsilon)), self.epsilon)
            self.fig.savefig(config.RL_DATA + "eps-training-{}.png".format(self.simulator.simulation_name()))
            plt.clf()
        ...
        self.fig.savefig(config.RL_DATA + "aoi-training-{}.png".format(self.simulator.simulation_name()))
        plt.clf()
